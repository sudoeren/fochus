...
import { Search, FileText, CheckSquare, Calendar, Settings, Trash2, Plus, Pin, Home } from 'lucide-react';
...
export const Spotlight: React.FC<SpotlightProps> = ({ isOpen, onClose, onNavigate }) => {
  ...
  const commands: SearchResult[] = [
    {
      ...
      subtitle: query.trim() ? `"${query.trim()}" başlıklı not oluştur` : 'Hızlıca yeni bir not ekle',
      type: 'command',
    ...
    },
    {
      ...
      subtitle: query.trim() ? `"${query.trim()}" görevini oluştur` : 'Hızlıca yeni bir görev ekle',
      type: 'command',
      icon: <Plus className="w-4 h-4" />,
      action: async () => {
        const title = query.trim() || 'Yeni Görev';
  ...
  const searchResults = useMemo(() => {
    ...
    return results;
  }, [query, notes, tasks, commands, navigationItems, onNavigate, onClose, addNote, addTask]);
