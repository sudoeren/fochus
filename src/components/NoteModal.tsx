...
export const NoteModal: React.FC<NoteModalProps> = ({ isOpen, onClose, editingNote, onEscapeToSpotlight }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      editingNote ? await updateNote(editingNote.id, dataToSave) : await addNote(dataToSave);
  ...
  const resetForm = () => {
    setNoteData({ title: '', content: '', plainContent: '', tags: [], hasReminder: false, reminderAt: undefined });
  };
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-zinc-800 w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="flex-1 overflow-y-auto p-8 scrollbar-thin scrollbar-thumb-gray-200 dark:scrollbar-thumb-zinc-700">
          ...
          <div className="min-h-[200px] mb-8">
            <RichTextEditor
              ...
              onChange={(content, plainText) => setNoteData(prev => ({ ...prev, content, plainContent: plainText }))}
          ...
          <div className="space-y-6 pt-6 border-t border-gray-100 dark:border-zinc-800">
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap items-center gap-2">
                {noteData.tags.map((tag, index) => (
                  <span key={index} className="inline-flex items-center gap-1 px-2.5 py-1 bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 rounded-lg text-sm border border-amber-100 dark:border-amber-900/30">
                    #{tag}
                    <button onClick={() => removeTag(tag)} className="hover:text-amber-900 dark:hover:text-amber-200"><X className="w-3 h-3" /></button>
                  </span>
                ))}
                <input
                  type="text"
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-4">
                <button
                  onClick={() => setNoteData(prev => ({ ...prev, hasReminder: !prev.hasReminder, reminderAt: !prev.hasReminder ? new Date() : undefined }))}
                ...
                >
                  {noteData.hasReminder ? <Bell className="w-4 h-4" /> : <BellOff className="w-4 h-4" />}
                ...
                {noteData.hasReminder && (
                  <input
                    ...
                    onChange={(e) => setNoteData(prev => ({ ...prev, reminderAt: e.target.value ? new Date(e.target.value) : undefined }))}
        ...
        <div className="p-6 border-t border-gray-100 dark:border-zinc-800 bg-white dark:bg-zinc-900 flex justify-between items-center">
          ...
          <div className="flex gap-3">
            <button onClick={onClose} className="px-6 py-2.5 text-sm font-medium text-gray-600 dark:text-zinc-400 hover:bg-gray-100 dark:hover:bg-zinc-800 rounded-xl transition-colors">
              İptal
            </button>
            <button onClick={handleSave} disabled={!noteData.title.trim()} className="px-8 py-2.5 text-sm font-medium bg-amber-500 hover:bg-amber-600 text-white rounded-xl shadow-lg shadow-amber-500/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
