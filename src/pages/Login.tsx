...
import { ArrowRight, CheckCircle2, User, Key, Mail, Shield, Zap } from 'lucide-react';
...
export const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // Simulate login/register - in a real app this would hit the backend
    onLogin();
  };

  const handleGuestLogin = () => {
    // Guest logic - maybe set a flag in localStorage if needed
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full py-3.5 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 rounded-xl font-medium hover:bg-zinc-800 dark:hover:bg-zinc-100 transition-colors flex items-center justify-center gap-2 mt-2 shadow-lg shadow-zinc-900/10"
            >
