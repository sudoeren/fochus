...
function setupDatabaseHandlers() {
  ...
  // Task List handlers
  ipcMain.handle('db-get-task-lists', async () => {
    try {
  ...
  ipcMain.handle('db-create-task-list', async (_event, data) => {
    try {
      // Get max order
  ...
  ipcMain.handle('db-update-task-list', async (_event, id: string, data) => {
    try {
  ...
  ipcMain.handle('db-delete-task-list', async (_event, id: string) => {
    try {
      return await prisma.taskList.update({
        where: { id },
        data: { isDeleted: true }
